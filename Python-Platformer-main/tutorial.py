...
def load_sprite_sheets(dir1, dir2, width, height, direction=False): #just to load sprite sheets, still need to use function
...
def get_block(size):
    ...
    rect = pygame.Rect(96, 0, size, size) #block in the png starts 96 pixels fromt the top of the screen
    surface.blit(image, (0, 0), rect)
    return pygame.transform.scale2x(surface)

class Player(pygame.sprite.Sprite): #pixel-perfec collision made easier with sprites
    ...
    def loop(self, fps): #will be call once per frame, is what actually moves the player
        ...
        if self.hit:
            self.hit_count += 1
        if self.hit_count > fps: #where you change how long fire animation is <- this line was originally //// if self.hit_count > fps * 2:  ////
            self.hit = False
            self.hit_count = 0
    ...
    def draw(self, win, offset_x): #this function draws the sprite, not animate
...
def get_background(name):
    image = pygame.image.load(join("assets", "Background", name)) # first load image
